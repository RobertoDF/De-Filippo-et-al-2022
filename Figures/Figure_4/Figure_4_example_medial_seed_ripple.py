...
window = (0.1, 0.2)
time_center = ripple_cluster_medial_seed.iloc[0]["Start (s)"]
...
for n, _ in enumerate(probes_list):
    ...
    axs[n + 1].plot(np.arange(-.1 + .0005, .2 - .0005, .001), out_hist[n]
                    / max(out_hist[n])*(axs[n+1].get_ylim()[1]-axs[n+1].get_ylim()[1]/50),
...
fs_lfp = 1250
lfp = lfp_per_probe[int(ripples["Probe number-area"].sort_values().unique()[0].split("-")[0])]
...
axs[0].plot(sample.time, filtered, alpha=.8,
            color=color_palette[round(((probes_list[0][0] - 6500) / (10000 - 6500)) * 255)])
