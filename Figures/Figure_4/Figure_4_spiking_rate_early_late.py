...
data = pd.melt(tot_summary_early[["Lateral seed", "Medial seed", "Session id"]],
               ...
               id_vars=["Session id"], var_name='Location seed', value_name='Spiking rate per 10 ms').groupby(
    ["Session id", "Location seed"]).mean().reset_index()

sns.boxplot(data=data, x="Location seed", y="Spiking rate per 10 ms", showfliers=False, ax=axs[0], palette=palette_ML,
