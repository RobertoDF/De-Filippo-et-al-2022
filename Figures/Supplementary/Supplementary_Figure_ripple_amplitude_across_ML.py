...
g = sns.pairplot(data=data, x_vars=["M-L (µm)", "A-P (µm)", "D-V (µm)"], y_vars=["Z-scored amplitude (mV)", "Amplitude (mV)"], kind="reg", plot_kws=dict(scatter_kws={"alpha": 0.6, "s":20, "color":(0.4,0.4,0.4)}, line_kws={"alpha": 0.8,  "color":"#D7263D", "linestyle": "--"}))
g.map_upper(corrfunc, y_pos=.8)
g.map_offdiag(corrfunc, y_pos=.8)

#plt.show()
plt.savefig(f"{output_folder_supplementary}/Figure 3-Figure supplement 4", dpi=300)
