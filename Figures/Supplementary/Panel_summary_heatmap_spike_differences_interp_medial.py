...
for session_id in tqdm([q[0] for q in list(spike_hists.keys()) if q[1] == "HPF"]):
    ...
    print(session_id, len(out_hist_strong) > 5, len(out_hist_common) > 5)
...
cbar.ax.set_label("colorbar_med") # needed to distinguish colobars
...
axs.text(-0.1, 1.15, "B", transform=axs.transAxes,
      fontsize=16, fontweight='bold', va='top', ha='right')

axs.text(0.5, 1.15, "Medial ripples", transform=axs.transAxes,
