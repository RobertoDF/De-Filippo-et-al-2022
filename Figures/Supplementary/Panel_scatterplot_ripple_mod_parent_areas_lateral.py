...
g.map_dataframe(sns.regplot, x='Firing rate (0-120 ms) lateral', y='Firing rate (100-0 ms) lateral',  scatter_kws=dict(s=5, alpha=.5))
...
def annotate(data, **kws):
    ...
    r, p = sp.stats.pearsonr(data['Firing rate (0-120 ms) lateral'], data['Firing rate (100-0 ms) lateral'])
    ax = plt.gca()
    if p == 0:
