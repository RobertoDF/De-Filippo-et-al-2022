...
g.map_dataframe(sns.regplot, x='Firing rate (0-120 ms) medial', y='Firing rate (100-0 ms) medial', scatter_kws=dict(s=5, alpha=.5))
...
def annotate(data, **kws):
    ...
    r, p = sp.stats.pearsonr(data['Firing rate (0-120 ms) medial'], data['Firing rate (100-0 ms) medial'])
    ax = plt.gca()
    if p == 0:
