...
with open(f'{output_folder_figures_calculations}/temp_data_figure_2.pkl', 'rb') as f:
    ...
    ripple_cluster_strong, ripple_cluster_weak, example_session = dill.load(f)
...
source = ripple_cluster_weak[ripple_cluster_weak["Start (s)"] == ripple_cluster_weak["Start (s)"].min()]

cc = []

for idx, r in ripple_cluster_weak.iterrows():
    ...
    if idx == ripple_cluster_weak.iloc[-1].name:
