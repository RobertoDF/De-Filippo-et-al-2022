...
source = Points(
    center_source[["A-P (µm)", "D-V (µm)", "L-R (µm)"]].values,
    radius=100,
    alpha=.6,
    colors="#F03A47",
...
other = Points(
    center_else[["A-P (µm)", "D-V (µm)", "L-R (µm)"]].values,
    radius=100,
    alpha=.6,
    colors="#353B3C",
    name=f"Correlations ∫Ripple")
...
scene.screenshot(name=f"{output_folder_figures_calculations}/Figure_2_temp_center.png", scale=4)
im = Image.open(f'{output_folder_figures_calculations}/Figure_2_temp_center.png')
width, height = im.size
im = im.crop((2750, 1000, width - 3000, height - 900))
im.save(f'{output_folder_figures_calculations}/Figure_2_brainrender_center_crop.png')
