...
def annotate(data, **kws):
    ...
    ax = plt.gca()
    if p == 0:
        ax.text(.1, y, f"{g}: R²={r ** 2:.2f}, p<0.0005", fontsize=5, transform=ax.transAxes, color=palette_parent_areas[g])
    else:
        ax.text(.1, y, f"{g}: R²={r ** 2:.2f}, p={2:.2e}", fontsize=5, transform=ax.transAxes, color=palette_parent_areas[g])
