...
g = sns.lmplot(data=summary_units_df_sub, x='Firing rate (0-50 ms)', y='Firing rate (120-0 ms)',
               ...
               palette=palette_parent_areas, scatter_kws=dict(alpha=.3, s=2),
...
def annotate(data, **kws):
    ...
    ax = plt.gca()
    if p < 0.001:
        ax.text(.1, y, f"{g}: R²={r ** 2:.2f}, p-value<0.001", fontsize=5, transform=ax.transAxes, color=palette_parent_areas[g])
    else:
        ax.text(.1, y, f"{g}: R²={r ** 2:.2f}, p-value={'{:.2e}'.format(p)}", fontsize=5, transform=ax.transAxes, color=palette_parent_areas[g])
