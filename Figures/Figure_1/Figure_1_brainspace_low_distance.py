...
pos_area = low_distance.groupby(["Session", "Probe number"])[["A-P (µm)", "D-V (µm)", "L-R (µm)"]].mean().values


colors = ["#0B4146", "#A3E9F0"] * (int(np.ceil(pos_area.shape[0]/2)))

probe_sphere2 = Points(
    pos_area,
    radius=140,
    alpha=0.5,
    colors=colors,
...
scene.screenshot(name=f"{output_folder_figures_calculations}/Figure_1_temp_low_distance_brainspace.png", scale=4)
im = Image.open(f'{output_folder_figures_calculations}/Figure_1_temp_low_distance_brainspace.png')
width, height = im.size
im = im.crop((2200, 1000, width - 2400, height - 1000))
im.save(f'{output_folder_figures_calculations}/Figure_1_brainrender_low_dist.png')
