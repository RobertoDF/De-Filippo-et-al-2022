...
sns.scatterplot(data=ripples_lags[(ripples_lags["Type"] == "High distance (µm)")], x="Lag (ms)", y="Duration (s)", color=sns.husl_palette(2)[0], s=0.1, alpha=0.5, ax=axs[0])#
axs[0].tick_params(labelbottom=False)
axs[0].xaxis.label.set_visible(False)
axs[1].xaxis.label.set_visible(False)
axs[1].tick_params(labelbottom=False)
sns.scatterplot(data=ripples_lags[(ripples_lags["Type"] == "Low distance (µm)")], x="Lag (ms)", y="Duration (s)", color=sns.husl_palette(2)[1], s=0.1, alpha=0.5, ax=axs[1])#
