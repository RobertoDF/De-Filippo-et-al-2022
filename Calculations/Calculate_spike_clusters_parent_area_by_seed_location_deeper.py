...
def process(session_id,  spike_clusters):
    print(session_id)
    ...
    HPF_spike_times = dict(zip(units[units[field_to_use_to_compare] == target_area].index,
    ...
    print(session_id, "Recording in each ML section:", np.any(ripples["L-R (µm)"].unique() < medial_lim) & np.any(ripples["L-R (µm)"].unique() > lateral_lim) & \
    ...
    if ripples.shape[0] > 0:
        for source_area, type_source in to_loop:
            ...
            for index, row in real_ripple_summary[real_ripple_summary["Location seed"] == "Medial"].iterrows():
                time_center = row["Start (s)"] + row[
                    row.index.str.contains('lag')].min()  # either sum zero or sum a negative value

                time_space_sub_spike_times_medial = {
                    cluster_id: spikes[(spikes > time_center - window[0]) & (spikes < time_center + window[1])] for
                    cluster_id, spikes in HPF_spike_times.items()}

                ripple_start_medial.append(time_center)

            ripple_start_lateral = []
            for index, row in real_ripple_summary[real_ripple_summary["Location seed"] == "Lateral"].iterrows():
                ...
                time_space_sub_spike_times_lateral = {
                    cluster_id: spikes[(spikes > time_center - window[0]) & (spikes < time_center + window[1])] for
                    cluster_id, spikes in HPF_spike_times.items()}

                ripple_start_lateral.append(time_center)

        spike_clusters[session_id] = [time_space_sub_spike_times_medial,
                                                                  time_space_sub_spike_times_lateral,
                                                                  ripple_start_medial,
                                                                  ripple_start_lateral,
                                                                  units]
...
with open(f"{output_folder_calculations}/spike_clusters_summary_per_parent_area_by_seed_location.pkl", "wb") as fp:
