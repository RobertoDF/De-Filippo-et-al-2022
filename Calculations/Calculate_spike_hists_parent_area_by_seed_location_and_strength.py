...
from Utils.Utils import acronym_to_main_area, clean_ripples_calculations, find_ripples_clusters_new, \
    batch_process_spike_hists_by_seed_location, process_spike_hists, batch_process_spike_hists_by_seed_location_and_strength
...
for session_id in tqdm(ripples_calcs.keys()):
    ...
    to_loop = []
    to_loop.append((ripples.groupby("Probe number-area").mean()['L-R (µm)'].idxmax(), "lateral"))
    to_loop.append((ripples.groupby("Probe number-area").mean()['L-R (µm)'].idxmin(), "medial"))
    ...
    if ripples.shape[0] > 0:
        for source_area, type_source in to_loop:
            ...
            real_ripple_summary["Local strong"] = real_ripple_summary["∫Ripple"] > real_ripple_summary[
                "∫Ripple"].quantile(.9)
            ...
            for target_area in tqdm(areas):
                lrs, out_hist_strong, out_hist_common = batch_process_spike_hists_by_seed_location_and_strength(func, real_ripple_summary, units, spike_times,
                                                                           target_area, field_to_use_to_compare, n_cpus, window_spike_hist)

                spike_hists[(session_id, target_area, type_source)] = [lrs, out_hist_strong, out_hist_common]

with open(f"{output_folder_calculations}/spike_hists_summary_per_parent_area_by_seed_location_and_strength.pkl", "wb") as fp:
